...
### 粒子群优化算法 (Particle Swarm Optimization, PSO) ###
# ====== PSO算法特有参数 ======
POPULATION_SIZE = 30        # 粒子群规模，建议范围：20-50
MAX_ITER = 50              # 最大迭代次数，建议范围：100-200
REASSIGN_RATIO = 0.3        # 每次迭代重新分配的订单比例，建议范围：0.2-0.5
COST_THRESHOLD = 1.2        # 成本容忍阈值，建议范围：1.1-1.5
UNASSIGNED_COST_FACTOR = 1.5 # 未分配订单的成本上限因子，建议范围：1.3-2.0
W = 0.7                     # 惯性权重，控制粒子保持原有移动趋势的程度，建议范围：0.5-0.9
C1 = 1.5                    # 个体学习因子，控制粒子向个体最优解移动的趋势，建议范围：1.0-2.0
C2 = 1.5                    # 社会学习因子，控制粒子向全局最优解移动的趋势，建议范围：1.0-2.0

def find_best_insertion(route, order, passengers, vehicle_start):
    """
    寻找将订单插入到路径中的最佳位置，返回插入位置和对应的成本。
    
    Args:
        route (list): 车辆当前的路径。
        order (dict): 包含 'pickup' 和 'dropoff' 节点的订单。
        passengers (dict): 所有乘客信息的字典。
        vehicle_start (tuple): 车辆的起始位置。
        
    Returns:
        tuple: 包含最佳插入位置 (i, j) 和对应成本的元组。如果没有可行位置，返回 (None, float('inf'))。
    """
    best_cost = float('inf')
    best_insertion = None
    ...
    for i in range(len(route) + 1):
        for j in range(i + 1, len(route) + 2):
            ...
            if is_feasible_insert(route, order, i, j, passengers, vehicle_start):
                ...
                # 计算新路径的成本
                cost = compute_cost_and_penalty(new_route, passengers, vehicle_start)
...
def particle_swarm_optimization():
    """
    粒子群优化算法主函数，用于求解动态车辆路径问题。
    该算法通过模拟鸟群的觅食行为来寻找最优解。
    """
    passengers_dict_pso = copy.deepcopy(passengers_dict)
    for pid, p in passengers_dict_pso.items():
    ...
    for _ in range(POPULATION_SIZE):
        particle = initialize_vehicles()
        # 随机分配订单到车辆
        orders = list(passengers_dict_pso.values())
        random.shuffle(orders)
        for order in orders:
            pid = order['passenger_id']
            vid = random.choice(list(particle.keys()))
            particle[vid]['route'].append(order['pickup'])
            particle[vid]['route'].append(order['dropoff'])
            particle[vid]['served'].add(pid)
        particles.append(particle)
    
    # 初始化每个粒子的个体最优解
    pbest_solutions = copy.deepcopy(particles)
    pbest_costs = [evaluate_total_cost(p, passengers_dict_pso) for p in particles]
    ...
    for iter in range(MAX_ITER):
        ...
        for i in range(POPULATION_SIZE):
            # 计算当前粒子的成本
            current_cost = evaluate_total_cost(particles[i], passengers_dict_pso)
            
            # 更新个体最优解
            if current_cost < pbest_costs[i]:
                pbest_costs[i] = current_cost
                pbest_solutions[i] = copy_solution(particles[i])
            
            # 更新全局最优解
            if current_cost < gbest_cost:
                gbest_cost = current_cost
                gbest_solution = copy_solution(particles[i])
            
            # 粒子移动：根据个体最优和全局最优更新粒子状态
            particles[i] = update_particle(
                particles[i], pbest_solutions[i], gbest_solution, passengers_dict_pso
            )
        
        print(f"  Current best cost: {gbest_cost:.2f}")
    
    return gbest_solution


def update_particle(particle, pbest, gbest, passengers):
    """
    根据个体最优和全局最优更新粒子状态。
    实现粒子的"移动"，即重新分配订单。
    """
    new_particle = copy_solution(particle)
    
    # 随机选择一定比例的订单进行重新分配
    reassign_ratio = 0.3
    served_pids = set()
    for vid in new_particle.keys():
        served_pids.update(new_particle[vid]['served'])
    
    # 从个体最优和全局最优中获取灵感
    pbest_pids = set()
    for vid in pbest.keys():
        pbest_pids.update(pbest[vid]['served'])
    
    gbest_pids = set()
    for vid in gbest.keys():
        gbest_pids.update(gbest[vid]['served'])
    
    # 确定要重新分配的订单
    reassign_pids = random.sample(list(served_pids), int(len(served_pids) * reassign_ratio))
    
    # 移除要重新分配的订单
    for vid in new_particle.keys():
        new_particle[vid]['route'] = [pt for pt in new_particle[vid]['route'] if pt[0] not in reassign_pids]
        new_particle[vid]['served'] = new_particle[vid]['served'] - set(reassign_pids)
    
    # 重新分配订单
    for pid in reassign_pids:
        ...
        # 尝试将订单插入到各个车辆
        for vid in new_particle.keys():
            insertion, cost = find_best_insertion(
                new_particle[vid]['route'], order, passengers, new_particle[vid]['location']
            )
        ...
        if best_vid is not None:
            i, j = best_insertion
            temp_route = new_particle[best_vid]['route'][:i] + [order['pickup']] + new_particle[best_vid]['route'][i:]
            new_particle[best_vid]['route'] = temp_route[:j] + [order['dropoff']] + temp_route[j:]
            new_particle[best_vid]['served'].add(pid)
...
def main():
    ...
    for name, algorithm_fn in algorithms.items():
        ...
        # 输出路径修改为 jingtai/results/particle_swarm_optimization
        folder_name = f"jingtai/results/particle_swarm_optimization"
